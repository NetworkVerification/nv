...
and showRecordTableP prec record =
  let fields =
    mkTr (StringMap.fold (fun k _ acc -> Printf.sprintf "%s%s" acc (mkTh k)) record "")
  in
  let values =
    mkTr
      (StringMap.fold
...
let checkAssertions (answers : Solution.t option) =
  (match answers with
  | None -> Jupyter_notebook.printf "<font color=\"green\">All assertions passed!</font>"
  | Some m ->
    let asserts =
    ...
    if all_pass
    then Jupyter_notebook.printf "<font color=\"green\">All assertions passed!</font>"
    else begin
...
let showSolutions ?(asTable = false) (solution : Solution.t option) =
  (match solution with
  | None -> Jupyter_notebook.printf "<font color=\"blue\">No solutions computed!</font>"
  | Some solution ->
    if asTable
    then begin
      let contents =
      ...
      AdjGraph.VertexMap.iter
        (fun node sol ->
          Jupyter_notebook.printf
            "<h3>Node %d</h3> <hr style=\"border-top: 1px solid #999; width=90%%;\">"
            node;
          showValue sol)
        solution.labels);
  Jupyter_notebook.display_formatter "text/html"
