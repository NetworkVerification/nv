...
let thStyle =
  "text-align: center; border: 1px solid black; border-collapse: collapse;"
;;
...
let mkTable ?(style = tableStyle) s =
  Printf.sprintf "<table style=\"%s\">%s</table>" style s
;;

let mkTh ?(style = thStyle) s =
  Printf.sprintf "<th style=\"%s\">%s</th>" style s
;;

let mkTd ?(style = tdStyle) s =
  Printf.sprintf "<td style=\"%s\">%s</td>" style s
;;

let mkTr ?(style = trStyle) s =
  Printf.sprintf "<tr style=\"%s\">%s</tr>" style s
;;
...
let rec containsMap v =
  ...
  | VTuple vs -> List.exists containsMap vs
  | VRecord map -> StringMap.exists (fun _ v -> containsMap v) map
;;

let rec showMap m =
  let binding_to_string k v = showValueP max_prec k, showValueP max_prec v in
  ...
  let tbl =
    List.fold_left
      (fun acc (k, v) ->
        ...
        Printf.sprintf "%s%s" acc (mkTr (mkTd k ^ mkTd v)))
      ""
      bs
  in
  mkTable (tbl ^ default)
...
and showSet m =
  ...
  let tbl =
    PrimitiveCollections.printList
      (fun (k, v) -> binding_to_string def k v)
      bs
      "{"
      ","
      "}"
  in
  if def then Printf.sprintf "*  %s" tbl else Printf.sprintf "%s" tbl
...
and showValueP prec v =
  ...
  | VTuple vs ->
    if List.is_empty vs
    then "[]"
    else PrimitiveCollections.printList (showValueP max_prec) vs "(" "," ")"
  | VOption None ->
  ...
  | VRecord map ->
    if containsMap v then showRecordTableP prec map else showRecordP prec map
...
and showRecordTableP prec record =
  let fields =
    mkTr
      (StringMap.fold
         (fun k _ acc -> Printf.sprintf "%s%s" acc (mkTh k))
         record
         "")
  in
  let values =
    mkTr
      (StringMap.fold
         (fun _ v acc -> Printf.sprintf "%s%s" acc (mkTd (showValueP prec v)))
         record
         "")
  in
...
and showRecordP prec record =
  Printf.sprintf
    "{%s}"
    (StringMap.fold
       (fun k v acc -> Printf.sprintf "%s%s = %s; " acc k (showValueP prec v))
       record
       "")
;;

let showValue v = Jupyter_notebook.printf "%s" (showValueP max_prec v)
    ...
    VarMap.fold
      (fun k v acc ->
        ...
        Printf.sprintf
          "%s%s"
          acc
          (mkTr (mkTd (Nv_datastructures.Var.name k) ^ mkTd vstring)))
      symbs
      ""
  in
...
let showAssertions asserts =
  ...
  let trueAssertions, falseAssertions =
    AdjGraph.VertexMap.partition (fun _ v -> v) asserts
  in
  ...
  let t =
    mkTr
      ~style:"background-color: #addeaa;"
      (mkTd "Passed"
      ^ mkTd
          (Collections.printList
             (fun (k, _) -> Printf.sprintf "%s" (string_of_int k))
             trueAssertions
             ""
             ","
             ""))
  ...
  | Some solution ->
    ...
    then begin
      let contents =
        AdjGraph.VertexMap.fold
          (fun node sol acc ->
            acc
            ^ mkTr
                (mkTd (Printf.sprintf "%d" node)
                ^ mkTd (showValueP max_prec sol)))
          solution.labels
          ""
      in
      ...
      AdjGraph.VertexMap.iter
        (fun node sol ->
          Jupyter_notebook.printf
            "<h3>Node %d</h3> <hr style=\"border-top: 1px solid #999; \
             width=90%%;\">"
            node;
          showValue sol)
        solution.labels);
