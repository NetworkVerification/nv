...
let get_record_labels map = List.map fst (StringMap.bindings map)
let get_record_entries map = List.map snd (StringMap.bindings map)

let same_labels map1 map2 =
  let cmp =
    List.compare String.compare (get_record_labels map1) (get_record_labels map2)
  in
  cmp = 0
;;
...
let get_type_with_label record_types (ferr : string -> unit) label =
  ...
  | None ->
    let msg =
      Printf.sprintf "Label %s does not appear in any declared record type!" label
    in
    ferr msg;
    failwith "Bad Label"
  | Some map -> map
...
let print_record ~(sep : string) (f : 'a -> string) (map : 'a StringMap.t) : string =
  let entries =
