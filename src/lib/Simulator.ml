...
let simulate_declaration
  ...
  | _ ->
    ...
    | DSolve solve ->
      let results =
        Srp.simulate_solve graph { ty = Env.empty; value = env } solve
      in
      begin
        ...
        | EVar x ->
          ...
          let bdd_base =
            BddMap.create
              ~key_ty:TNode
              (Generators.default_value (oget solve.aty))
          in
          let bdd_full =
          ...
          in
          let mapval =
            avalue (vmap bdd_full, Some xty, solve.var_names.espan)
          in
          { state with
...
let simulate_declarations ~(throw_requires : bool) (decls : declarations)
    : Solution.t
  =
  ...
  let final_state =
    List.fold_left
      (simulate_declaration ~throw_requires graph)
      empty_state
      decls
  in
  ...
  let assertions =
    ...
    in
    pad
      (num_asserts - List.length final_state.assertions)
