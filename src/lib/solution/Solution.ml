...
let print_masked_type unmasked_ty sol =
  ...
  let rec construct_string ty mask =
    ...
    | TTuple tys, VTuple ms ->
      Printf.sprintf "(%s)" @@ BatString.concat "," @@ List.map2 construct_string tys ms
    ...
    | (TVar _ | QVar _ | TArrow _), _ -> failwith "print_masked_type: Nonsense type"
  in
  match sol.mask with
...
let rec print_masked mask v =
  ...
  | VOption (Some v), VOption (Some m) -> Printf.sprintf "(Some %s)" @@ print_masked m v
  | VTuple vs, VTuple bs ->
    Printf.sprintf "(%s)" @@ BatString.concat "," @@ List.map2 print_masked bs vs
