...
let start_solver (params : string list) =
  ...
  let pid =
    BatUnix.create_process
      solver
      (Array.of_list (solver :: params))
      solver_in
      solver_out
      solver_out
  in
  ...
  { pid; nvin = cin; nvout = cout; running = true }
;;
...
let get_reply (solver : solver_proc) : string option =
  try Some (input_line solver.nvin) with
  | End_of_file -> None
;;
...
let get_reply_until marker (solver : solver_proc) : string list =
  let rec loop acc =
    try
      ...
      if s = marker then BatList.rev (s :: acc) else loop (s :: acc)
    with
    | End_of_file -> BatList.rev acc
  in
  loop []
;;
