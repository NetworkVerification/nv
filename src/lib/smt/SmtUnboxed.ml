...
module Unboxed : SmtEncodingSigs.ExprEncoding = struct
      ...
      | AtMost _ ->
        ...
        | [e1; e2; e3] ->
          ...
          | ETuple es ->
            let zes =
              BatList.map
                (fun e -> (encode_exp_z3_single ~arith:true descr env e).t)
                es
            in
            (match e2.e with
            | ETuple es ->
  ...
  and encode_exp_z3 descr env (e : exp) : term list =
    ...
    | EOp (op, es) ->
      ...
      | Eq, [e1; e2] ->
        ...
        let componentwise_eqs =
          lift2
            (fun ze1 ze2 -> mk_eq ze1.t ze2.t |> mk_term ~tloc:e.espan)
            ze1
            ze2
        in
        [ List.fold_left
    ...
    | EIf (e1, e2, e3) ->
      ...
      lift2
        (fun ze2 ze3 ->
          mk_ite_fast guard.t ze2.t ze3.t |> mk_term ~tloc:e.espan)
        zes2
        zes3
    | ELet (x, e1, e2) ->
      ...
      let za =
        mk_constant env xs sorts ~cloc:e.espan ~cdescr:(descr ^ "-let")
      in
      let ze1 = encode_exp_z3_single descr env e1 in
      let zes2 = encode_exp_z3 descr env e2 in
  ...
  and encode_branches_aux_z3 descr env names bs (t : ty) =
    ...
    | (p, e), bs ->
      ...
      lift2
        (fun ze accze ->
          mk_ite_fast guard.t ze.t accze.t |> mk_term ~tloc:e.espan)
        zes
  ...
  and encode_value_z3_single ?(arith = false) descr env (v : Syntax.value)
      : term
    =
    match v.v with
