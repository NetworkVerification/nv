...
struct
  ...
  let add_symbolic_constraints env requires sym_vars =
    ...
    VarMap.iter
      (fun v e ->
        ...
        let symb =
          mk_constant env names sort ~cdescr:"Symbolic variable decl"
        in
        match sort with
        | IntSort ->
    ...
    let rec loop merge acc =
      ...
      | EFun { arg = x; argty = Some xty; body = exp; _ } ->
        ...
        | _ ->
          ...
          in
          let names =
            create_strings (Printf.sprintf "%s-result" str) (oget exp.ety)
          in
          let results =
            lift2 (mk_constant env) names (oget exp.ety |> ty_to_sorts)
          in
  ...
  let rec encode_z3_trans_aux trans acc =
    ...
    | EFun { arg = x; argty = Some xty; body = exp; _ } ->
      ...
      | _ ->
        ( BatList.rev_map
            (fun (x, xty) -> x, SmtUtils.ty_to_sort xty)
            ((x, xty) :: acc)
        , exp ))
    | _ -> failwith "internal error"
  ;;
  ...
  let encode_solve env graph count { aty; var_names; init; trans; merge } =
    ...
    for i = 0 to nodes - 1 do
      ...
      let einit_i =
        Nv_interpreter.InterpPartial.interp_partial_fun einit [node]
      in
      let init =
        encode_z3_init (Printf.sprintf "init%d-%d" count i) env einit_i
      in
      ...
      let l =
        lift2
          (fun lbl s -> mk_constant env (create_vars env "" lbl) s)
          lbl_iv
          attr_sort
      in
      ignore
        (lift2
           (fun l merged ->
             SmtUtils.add_constraint env (mk_term (mk_eq l.t merged.t)))
           l
           merged);
      labelling.(i) <- l
    done;
    ...
    AdjGraph.EdgeMap.iter
      (fun (i, _) x ->
        ...
        BatList.iter2
          (fun label x ->
            SmtUtils.add_constraint env (mk_term (mk_eq label.t x.t)))
          (to_list label)
          x)
      !trans_input_map
  ;;
  ...
  let encode_z3 (decls : Syntax.declarations) : SmtUtils.smt_env =
    ...
    let assertions =
      get_asserts decls |> List.map InterpPartial.interp_partial
    in
    ...
    | _ ->
      ...
      let assert_vars =
        List.mapi
          (fun i eassert ->
            let z3_assert =
              encode_exp_z3 "" env eassert |> to_list |> List.hd
            in
            let assert_var =
              mk_constant env (Printf.sprintf "assert-%d" i) (ty_to_sort TBool)
            in
            SmtUtils.add_constraint
              env
              (mk_term (mk_eq assert_var.t z3_assert.t));
            assert_var)
          assertions
      in
