...
let op_to_ocaml_string op =
  ...
  | TGet _ | TSet _ -> failwith "Todo: TGet/TSet"
;;
...
let rec pattern_to_ocaml_string pattern =
  ...
  | PTuple ps ->
    ...
    Collections.printListi
      (fun i p -> Printf.sprintf "%s = %s" (proj_rec i n) (pattern_to_ocaml_string p))
      ps
      "{"
      "; "
      "}"
  ...
  | PEdge (p1, p2) ->
    Printf.sprintf "(%s, %s)" (pattern_to_ocaml_string p1) (pattern_to_ocaml_string p2)
;;
...
and track_tuples_pattern p =
  ...
  | POption None
  | PWild | PVar _ | PUnit | PBool _ | PInt _ | PRecord _ | PNode _ | PEdge _ -> ()
;;
...
let rec value_to_ocaml_string v =
  ...
  | VTuple vs ->
    let n = BatList.length vs in
    Collections.printListi
      (fun i v -> Printf.sprintf "%s = %s" (proj_rec i n) (value_to_ocaml_string v))
      vs
      "{"
      "; "
      "}"
...
and exp_to_ocaml_string e =
  ...
  | EApp (e1, e2) ->
    Printf.sprintf "(%s) (%s)" (exp_to_ocaml_string e1) (exp_to_ocaml_string e2)
  | EIf (e1, e2, e3) ->
    Printf.sprintf
      "(if %s then\n %s else\n %s)"
      (exp_to_ocaml_string e1)
  ...
  (* default to true *)
  | EIgnore _ -> value_to_ocaml_string (vbool true)
  ...
  | MMerge ->
    ...
    | e1 :: e2 :: e3 :: rest ->
      ...
      let opt =
        ...
        | [el0; el1; er0; er1] ->
          Printf.sprintf
            "~opt:(Obj.magic (Some (%s,%s,%s,%s)))"
            (exp_to_ocaml_string el0)
            (exp_to_ocaml_string el1)
            (exp_to_ocaml_string er0)
            (exp_to_ocaml_string er1)
        | _ -> ""
      in
      Printf.sprintf
        "(Obj.magic (let %s = %s in \n\
...
let compile_decl decl =
  ...
  | DSymbolic (x, _) ->
    let s =
      Printf.sprintf
        "let %s = S.get_symb record_cnstrs record_fns %d"
        (varname x)
        !symbolic_counter
    in
    incr symbolic_counter;
    s
  | DLet (x, _, e) -> Printf.sprintf "let %s = %s" (varname x) (exp_to_ocaml_string e)
    ...
    begin
      ...
      | EVar x ->
        ...
        | Some attr ->
          ...
          ignore (get_fresh_type_id type_store TNode);
...
let compile_decls decls =
  ...
  let embeddings =
    "let _ = Embeddings.build_embed_cache record_fns\n\
...
let set_entry (name : string) =
  Printf.sprintf
    "let () = SrpNative.srp := Some (module %s:SrpNative.CompleteSRPSig)"
    name
;;
...
let compile_ocaml name net =
  ...
  let src_dir =
    try Sys.getenv "NV_BUILD" ^ name with
    | Not_found ->
      failwith
        "To use compiler, please set environment variable NV_BUILD to a directory in \
         which to generate build files. Use something outside the nv directory."
  in
