    ...
    | [pred; f; m] ->
      let pred_closure =
        ...
        | EFun predF ->
          ...
          let freeVars =
            Syntax.free_ty
              (BatSet.PSet.singleton ~cmp:Var.compare predF.arg)
              predF.body
          in
          ...
          Collections.printList
            (fun (x, ty) ->
              Printf.sprintf
                "(%s,%d)"
                (varname x)
                (get_fresh_type_id type_store ty))
            freeList
            "("
            ","
            ")"
  ...
  let header =
    Printf.sprintf
      ...
      \ open Nv_lang\n\
       open Syntax\n\
       open Nv_compile\n\n\
       module %s (S: Symbolics.PackedSymbolics) \
       (SIM:SrpNative.SrpSimulationSig): SrpNative.NATIVE_SRP = struct\n"
      name
  in
  let ocaml_decls =
...
let compile_ocaml name net =
  ...
  let src_dir =
    ...
    | Not_found ->
      failwith
        "To use compiler, please set environment variable NV_BUILD to a \
