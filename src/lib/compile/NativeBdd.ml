...
let map (op_key : int * 'f) (vty_new_id : int) (f : 'a1 -> 'a2) (vmap : t) : t =
  ...
  let op =
    ...
    | Some op -> op
  in
  { bdd = User.apply_op1 op vdd, kty
  ; key_ty_id = vmap.key_ty_id
...
let merge ?(opt = None) (op_key : int * 'f) f (vmap1 : t) (vmap2 : t) =
  ...
  in
  { vmap1 with
    bdd = User.apply_op2 op (fst vmap1.bdd) (fst vmap2.bdd), snd vmap1.bdd
  }
;;
  ...
  let rec loop list clos acc =
    ...
    | [y] ->
      Env.update
        acc
        y
        (BddFunc.eval_value (embed_value_id (snd clos) (fst clos)))
    | y :: ys ->
      ...
      let env =
        Env.update
          acc
          y
          (BddFunc.eval_value (embed_value_id (snd elt) (fst elt)))
      in
...
let mapIf
  ...
  let cfg = Cmdline.get_cfg () in
  let f_embed x =
    f (unembed_value_id vmap.val_ty_id x) |> embed_value_id vty_new_id
  in
  let g b v =
    if Mtbdd.get b then f_embed (Mtbdd.get v) |> Mtbdd.unique B.tbl else v
  in
  let pred =
