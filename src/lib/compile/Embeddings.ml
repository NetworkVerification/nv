...
let rec embed_value (record_fns : int * int -> 'a -> 'b) (typ : Syntax.ty)
  ...
  | TVar _ | QVar _ ->
    failwith ("TVars and QVars should not show up here: " ^ PrintingRaw.show_ty typ)
;;

(** Takes an NV value of type typ and returns an OCaml value.*)
let rec unembed_value
    (record_cnstrs : int -> 'c)
    (record_proj : int * int -> 'a -> 'b)
    (typ : Syntax.ty)
    : Syntax.value -> 'v
  =
  match typ with
  | TUnit -> fun _ -> Obj.magic ()
  | TBool ->
    fun v ->
      (match v.v with
      | VBool b -> Obj.magic b
      | _ ->
        failwith
          (Printf.sprintf
             "mistyped value %s at type %s\n"
             (PrintingRaw.show_value ~show_meta:false v)
