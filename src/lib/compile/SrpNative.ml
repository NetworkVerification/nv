...
module type CompleteSRPSig = functor (S : PackedSymbolics) (SIM : SrpSimulationSig) ->
  NATIVE_SRP
...
module SrpSimulation (G : Topology) : SrpSimulationSig = struct
    ...
    let do_neighbor (_, initial_attribute) (s, todo) neighbor =
      ...
      | None ->
        ...
        then AdjGraph.VertexMap.add neighbor (new_entry, n_new_attribute) s, todo
          ...
          AdjGraph.VertexMap.add neighbor (new_entry, n_new_attribute) s, neighbor :: todo
  ...
  let simulate_solve attr_ty_id name init trans merge =
    ...
    let bdd_base =
      NativeBdd.create
        ~key_ty_id:(Collections.TypeIds.get_id CompileBDDs.type_store TNode)
        ~val_ty_id:attr_ty_id
        default
    in
    let bdd_full =
      AdjGraph.VertexMap.fold
