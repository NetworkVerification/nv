...
(* We need to make a "copy" of BddMap functions here, due to using a different manager/table. *)
...
let value_to_bdd (v: value) : Bdd.vt =
  let rec aux v idx =
    ...
    | VInt i ->
      B.mk_int (Integer.size i) (Integer.to_int i) idx, idx + Integer.size i
    | VTuple vs ->
...
type t =
  ...
  | BMap of mtbdd (* MTBDD map, for now leafs are values *)
...
let rec equal_t x y =
  ...
  | BBool b1, BBool b2 ->
    Bdd.is_equal b1 b2
  | BInt i1, BInt i2 ->
    Array.for_all2 Bdd.is_equal i1 i2
  | BOption (t1, x), BOption (t2, y) ->
    Bdd.is_equal t1 t2 && equal_t x y
  | Tuple ts1, Tuple ts2 ->
    List.for_all2 equal_t ts1 ts2
  | BMap m1, BMap m2 ->
    Mtbdd.is_equal (fst m1) (fst m2)
  | Value v1, Value v2 ->
  ...
  | VInt i ->
    let bs =
      Array.init (Integer.size i) (fun j ->
          let bit = B.get_bit (Integer.to_int i) j in
          bdd_of_bool bit)
    in
    BInt bs
  | VNode n ->
...
let eqMap x y : Bdd.vt =
  ...
  | BMap m1, BMap m2 ->
    (Mapleaf.mapleaf2
       (fun v1 v2 -> equal_values ~cmp_meta:false
           (Mtbdd.get v1) (Mtbdd.get v2) |> Mtbdd.unique B.tbl_bool) (fst m1) (fst m2))
    |> bdd_of_mtbdd
  | BMap m, Value v | Value v, BMap m ->
    (Mapleaf.mapleaf1
       (fun vm -> equal_values ~cmp_meta:false
           (Mtbdd.get vm) v |> Mtbdd.unique B.tbl_bool) (fst m))
    |> bdd_of_mtbdd
...
let isMap x =
  match x with
  | BMap _ -> true
  | BBool _ | BInt _ | BOption _
  | Tuple _ | Value _ -> false

(** Equality between [t] values*)
let rec eq (x: t) (y: t) : t =
  if (isBdd x) || (isBdd y) then (*lift to bdd *)
    BBool (eqBdd (eval_value_id x) (eval_value_id y))
  else if (isMap x) || (isMap y) then (*lift to mtbdd*)
    BBool (eqMap x y)
  else
    match x,y with
    | Value v1, Value v2 ->
      Value(if equal_values ~cmp_meta:false v1 v2 then vbool true
            else vbool false)
    | Tuple bs1, Tuple bs2 ->
      eq_list bs1 bs2 (Value (vbool true))
    | BOption (tag1, b1), BOption (tag2, b2) ->
...
let eval_branch_mapLift env matches key_ty =
  ...
  let result =
    Array.fold_left (fun acc (k,v) ->
        match v with
        | None -> acc (* not a match*)
        | Some (matchEnv, c) -> (* if there is a match with env under condition c*)
          let cond =
            match c with
            | None -> k (*if there is no condition then just use k*)
            | Some c -> Bdd.dand k c (*otherwise their conjunction*)
          in
          (match acc with
           | None -> (* If there is no other match*)
             let newEnv =
               Env.map matchEnv (fun v ->
                   match v with
                   | Value v ->
                     BMap (create key_ty v)
           ...
           | Some (accEnv, accC) ->
             ...
             let newEnv =
               Env.combineValues (fun vacc v ->
                   match vacc, v with
                   | BMap m, Value v ->
                     (* For keys that satisfy the current condition
                        return v otherwise return whatever was in them map*)
                     BMap (Mapleaf.mapleaf2 (fun b vcur ->
                         (if (Mtbdd.get b) then v else Mtbdd.get vcur) |>
                         Mtbdd.unique B.tbl_nv) (wrap_mtbdd cond) (fst m), (snd m))
                   | _, _ -> failwith "Invalid combination") accEnv matchEnv
             in
             let newCond =
               match accC with
               | None -> cond
...
let eval_tget lo hi x =
  ...
  | BMap m ->
    BMap (Mapleaf.mapleaf1
            (fun v -> eval_tget_v lo hi (Mtbdd.get v) |> Mtbdd.unique B.tbl_nv) (fst m),
          snd m)
                ...
                | VTuple elts ->
                  if lo = hi then
                    ...
                    (match v.v with
                     ...
                     | _ -> failwith "Bad TSet")
                | _ -> failwith "Expected a tuple") |> Mtbdd.unique B.tbl_nv
            ) (fst m), snd m)
  | _ -> failwith "Impossible"
...
let rec eval (env: t Env.t) (e: exp) : t =
  ...
  | ESome e ->
    (match eval env e with
     ...
     | BMap m -> BMap (Mapleaf.mapleaf1 (fun v -> voption (Some (Mtbdd.get v)) |> Mtbdd.unique B.tbl_nv) (fst m), snd m)
...
and eval_branch env (g : t) p =
  ...
  | PInt pi, Value v ->
    (match v.v with
     ...
     | _ -> None)
  | PInt pi, BInt bi ->
    if Integer.size pi <> Array.length bi then
  ...
  | POption None, BOption (tag, _) ->
    if Bdd.is_false tag then (* if tag is a false constant*)
      Some (env, None)
    else if Bdd.is_true tag then (*if tag is a true constant*)
      None
  ...
  | POption (Some p), BOption (tag, v) ->
    if Bdd.is_false tag then (* if tag is a false constant*)
      ...
      match eval_branch env v p with (*check that inner-pattern matches*)
      | None -> None
      | Some res -> Some res
    else (*if non-constant*)
      (match eval_branch env v p with (*check that inner-pattern matches*)
...
and eval_ite env e1 e2 e3 =
  ...
  let g = match g with
    | Value _ | BBool _ -> g
    | BMap m -> BBool (bdd_of_mtbdd (Mapleaf.mapleaf1 (fun v ->
        match (Mtbdd.get v).v with
...
and eval_ite_bdd_guard key_ty b v1 v2 =
  ...
  | BMap m1, BMap m2 ->
    let res = User.map_op3
        ~special:(fun bdd1 bdd2 bdd3 ->
            if Vdd.is_cst bdd1
            then Some(if Mtbdd.dval bdd1 then bdd2 else bdd3)
            else None
          )
        (fun b1 b2 b3 -> (if Mtbdd.get b1 then Mtbdd.get b2 else Mtbdd.get b3) |> Mtbdd.unique B.tbl_nv)
        (wrap_mtbdd b) (fst m1) (fst m2)
    in
    BMap (res, snd m1)
  | BMap m1, Value v2 ->
    BMap (Mapleaf.mapleaf2
            (fun b v -> (if Mtbdd.get b then Mtbdd.get v else v2)
                        |> Mtbdd.unique B.tbl_nv) (wrap_mtbdd b) (fst m1), snd m1)
  | Value v1, BMap m2 ->
    BMap (Mapleaf.mapleaf2
            (fun b v -> (if Mtbdd.get b then v1 else Mtbdd.get v)
                        |> Mtbdd.unique B.tbl_nv) (wrap_mtbdd b) (fst m2), snd m2)
  | Value v1, b2 when isBdd b2 = true ->
