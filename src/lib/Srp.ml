  ...
  let do_neighbor (_, initial_attribute) (s, todo) n =
    ...
    | Some old_attribute_from_x ->
      ...
      let compare_routes =
        Interp.interp_closure
          merge
          [neighbor; old_attribute_from_x; n_incoming_attribute]
      in
      (* Printf.printf "compare_routes is:%s\n" (Printing.value_to_string compare_routes); *)
      let dummy_new =
        Interp.interp_closure
          merge
          [neighbor; n_incoming_attribute; n_incoming_attribute]
      in
      ...
      then (
        ...
        in
        let newTodo =
          if equal_values ~cmp_meta:false n_old_attribute best
          then todo
          else n :: todo
        in
