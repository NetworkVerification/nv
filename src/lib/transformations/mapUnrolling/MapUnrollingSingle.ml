...
let ty_transformer (target : ty) (keys : keys) _ ty =
  ...
  | TMap (_, vty) ->
    ...
    then (
      ...
      Some
        (TTuple (List.make (List.length const_keys + List.length symb_keys) vty)))
    else None
  (* | QVar _ -> Some ty *)
  | _ -> None
;;
      ...
      let index =
        ...
        | EVar v ->
          begin
            ...
            | None ->
              failwith
              @@ "Encountered non-constant, non-symbolic variable key whose \
                  type is not TNode or TEdge: "
        ...
        | _ ->
          failwith
          @@ "Encountered non-constant, non-variable key whose type is not \
              TNode or TEdge: "
          ^ Printing.exp_to_string k
      in
...
let exp_transformer
    (target : ty)
    (keys : keys)
    (recursors : Transformers.recursors)
    e
  =
    ...
    begin
      ...
      | MSet, [map; k; setval] when has_target_type map ->
        ...
        let mk_op index =
          aexp
            ( eop
                (TSet (unrolled_size, index, index))
                [unrolled_map; unrolled_setval]
            , unrolled_ety
            , e.espan )
        in
        Some (unroll_get_or_set keys mk_op unrolled_ety e.espan k)
      ...
      | MMerge, f :: map1 :: map2 :: _
        when has_target_type map1 && has_target_type map2 ->
        ...
        Some
          (ematch
             ...
             (addBranch
                ...
                (aexp (etuple result, Some unrolled_target, e.espan))
                emptyBranch))
      | (MFoldNode, [f; map; acc] | MFoldEdge, [f; map; acc])
        when has_target_type map ->
        (* fold f acc m ->
           (f (f acc k1 v1) k2 v2)
        *)
        let key_ty, val_ty =
        ...
        let freshvars =
          List.map
            (fun k -> k, Var.fresh "UnrollingFoldVar")
            (symb_var_keys @ const_keys)
        in
        let pattern = PTuple (List.map (fun (_, var) -> PVar var) freshvars) in
        let fold_vars =
          List.map
            (fun (k, v) -> k, aexp (evar v, Some val_ty, e.espan))
            freshvars
        in
        let result =
          List.fold_left
            (fun acc (k, v) ->
              let app1 =
                aexp
                  ( eapp f' k
                  , Some (TArrow (val_ty, TArrow (acc_ty, acc_ty)))
                  , e.espan )
              in
              let app2 =
                aexp (eapp app1 v, Some (TArrow (acc_ty, acc_ty)), e.espan)
              in
      ...
      | ( ( MCreate
        ...
        , _ ) ->
        failwith
        @@ "Failed to unroll map: Incorrect number of arguments to map \
            operation : "
...
let map_back_transformer (keys : keys) _ (sol : Solution.t) v orig_ty =
  ...
  | VTuple vs, TMap (kty, vty) ->
    ...
    let v_bindings =
      List.combine
        (List.map (fun v -> List.assoc v sol.symbolics) symb_keys)
        symb_vs
    in
