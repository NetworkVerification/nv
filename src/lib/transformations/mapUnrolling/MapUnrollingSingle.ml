...
let has_target_type (target : ty) (e : exp) : bool = equal_inner_tys target (oget e.ety)

let get_index_const (keys : keys) (k : exp) =
  let const_keys, _ = keys in
  try
    let index, _ = List.findi (fun _ e -> compare_es e k = 0) const_keys in
    Some index
  with
  | _ -> None
;;
...
let get_index_symb (keys : keys) (symb : var) =
  ...
  try
    let index, _ = List.findi (fun _ s -> Var.equal s symb) symb_keys in
...
let rec unroll_get_or_set
  ...
  | None ->
    ...
    let const_branches =
      const_keys
      |> List.mapi (fun i k -> exp_to_pattern k, mk_op i)
      |> List.fold_left (fun acc (p, x) -> addBranch p x acc) emptyBranch
    in
      ...
      let index =
        ...
        | EVar v ->
          begin
            ...
            | None ->
              failwith
              @@ "Encountered non-constant, non-symbolic variable key whose type is not \
                  TNode or TEdge: "
              ^ Printing.exp_to_string k
              ^ ", type: "
              ^ Printing.ty_to_string (oget k.ety)
          end
        | _ ->
          failwith
          @@ "Encountered non-constant, non-variable key whose type is not TNode or \
              TEdge: "
          ^ Printing.exp_to_string k
      in
      (* List of symbolics with a strictly lower index *)
      let preceding_symb_keys = List.take index symb_keys in
      let big_if =
        List.fold_left
          (fun acc v ->
            let cmp =
  ...
  | EOp (op, es) ->
    begin
      ...
      | MGet, [map; k] when has_target_type map ->
        ...
        let mk_op index =
          aexp
            ( eop (TGet (unrolled_size, index, index)) [unrolled_map]
            , unrolled_ety
            , e.espan )
        in
      ...
      | MSet, [map; k; setval] when has_target_type map ->
        ...
        let mk_op index =
          aexp
            ( eop (TSet (unrolled_size, index, index)) [unrolled_map; unrolled_setval]
            , unrolled_ety
            , e.espan )
        in
        Some (unroll_get_or_set keys mk_op unrolled_ety e.espan k)
      | MMap, [f; map] when has_target_type map ->
        ...
        in
        let pattern = PTuple (List.map (fun var -> PVar var) freshvars) in
        let freshvars =
          List.map (fun var -> aexp (evar var, Some val_ty, e.espan)) freshvars
        in
        let result =
          List.map (fun var -> aexp (eapp f' var, Some val_ty, e.espan)) freshvars
        in
        Some
          (ematch
             (unroll_exp map)
             (addBranch
                pattern
                (aexp (etuple result, Some unrolled_target, e.espan))
                emptyBranch))
      | MMapFilter, [p; f; map] when has_target_type map ->
        ...
        let all_keys =
          List.map (fun v -> aexp (evar v, Some key_ty, e.espan)) symb_keys @ const_keys
        in
        let freshvars = List.map (fun _ -> Var.fresh "UnrollingMapFilterVar") all_keys in
        let pattern = PTuple (List.map (fun var -> PVar var) freshvars) in
        let val_ty =
          match target (* Type of the map we're unrolling *) with
          | TMap (_, ty) -> ty
        ...
        let freshvars =
          List.map (fun var -> aexp (evar var, Some val_ty, e.espan)) freshvars
        in
        let result = List.map2 make_result all_keys freshvars in
        Some
          (ematch
             (unroll_exp map)
             (addBranch
      ...
      | MMerge, f :: map1 :: map2 :: _ when has_target_type map1 && has_target_type map2
        ...
        let freshvars1, freshvars2 =
          let lst = List.make (List.length symb_keys + List.length const_keys) () in
        ...
        let result =
          List.map2
            (fun var1 var2 ->
              ...
              let fst_app = aexp (eapp f' v1, Some (TArrow (val_ty, val_ty)), e.espan) in
        ...
        Some
          (ematch
             ...
             (addBranch
                ...
                (aexp (etuple result, Some unrolled_target, e.espan))
                emptyBranch))
      | (MFoldNode, [f; map; acc] | MFoldEdge, [f; map; acc]) when has_target_type map ->
        (* fold f acc m ->
           (f (f acc k1 v1) k2 v2)
        *)
        let key_ty, val_ty =
        ...
        in
        let pattern = PTuple (List.map (fun (_, var) -> PVar var) freshvars) in
        let fold_vars =
        ...
        let result =
          List.fold_left
            (fun acc (k, v) ->
              let app1 =
                aexp (eapp f' k, Some (TArrow (val_ty, TArrow (acc_ty, acc_ty))), e.espan)
              in
      ...
      | ( ( MCreate
        ...
        , _ ) ->
        failwith
        @@ "Failed to unroll map: Incorrect number of arguments to map operation : "
...
let map_back_transformer (keys : keys) _ (sol : Solution.t) v orig_ty =
  ...
  | VTuple vs, TMap (kty, vty) ->
    ...
    let v_bindings =
      List.combine (List.map (fun v -> List.assoc v sol.symbolics) symb_keys) symb_vs
    in
...
let make_toplevel
  ...
  toplevel_transformer
    ~name:"MapUnrolling"
    (ty_transformer target keys)
    pattern_transformer
    value_transformer
    (exp_transformer target keys)
    (map_back_transformer keys)
    (mask_transformer keys)
;;

let unroll_declarations target keys =
  make_toplevel target keys Transformers.transform_declarations
;;
