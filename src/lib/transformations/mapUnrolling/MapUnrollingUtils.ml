...
let add_key mty symbolics (const_keys, symb_keys) keyo =
  ...
  | Some key ->
    ...
    then ExpSet.add key const_keys, symb_keys
      ...
      | _ ->
        failwith
        @@ "Encountered non-variable, non constant map key whose type isn't \
            node or edge: "
...
let rec collect_in_exp (symbolics : var list) (exp : Syntax.exp) (acc : maplist)
  ...
  | ELet (_, e1, e2) | EApp (e1, e2) ->
    acc |> collect_in_exp e1 |> collect_in_exp e2
  | EIf (e1, e2, e3) ->
...
let collect_in_decl (symbolics : var list) (d : declaration) (acc : maplist)
    : maplist
  =
...
let add_keys_for_nodes_and_edges decls maplist =
  let make_node n =
    aexp
      ( e_val (avalue (vnode n, Some TNode, Span.default))
      , Some TNode
      , Span.default )
  in
  let make_edge (i, j) =
    aexp
      ( e_val (avalue (vedge (i, j), Some TEdge, Span.default))
      , Some TEdge
      , Span.default )
  in
  let nodes =
