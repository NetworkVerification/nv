...
let rename_declaration_groups (dgs : declaration_groups) =
...
let rename_declarations_or_group d_or_g =
  match d_or_g with
  | Decls d ->
    let d, f = rename_declarations d in
    Decls d, f
  | Grp g ->
    let g, f = rename_declaration_groups g in
    Grp g, f
;;
