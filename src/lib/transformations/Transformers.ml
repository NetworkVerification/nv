...
and transform_pattern
  ...
  | None ->
    ...
    | PRecord pmap, TRecord tmap ->
      PRecord
        (StringMap.mapi (fun l p -> transform_pattern p (StringMap.find l tmap)) pmap)
    | _, _ ->
      failwith
      @@ Printf.sprintf
    ...
    | None ->
      ...
      | EMatch (test, branches) ->
        ematch
          (transform_exp test)
          (mapBranches
             (fun (p, b) -> transform_pattern p (oget test.ety), transform_exp b)
             branches))
  in
...
let transform_symbolic ~(name : string) (transformers : transformers) (x, toe) =
  let toe' =
    match toe with
    | Ty ty -> Ty (transform_ty ~name transformers ty)
    | Exp e -> Exp (transform_exp ~name transformers e)
  in
  x, toe'
;;
...
let transform_decl ~(name : string) (transformers : transformers) (d : declaration) =
  ...
  | DSolve { aty; var_names; init; trans; merge; interface; decomp; global } ->
    ...
    let decomp =
      ...
      | None -> None
    in
    let global = omap transform_exp global in
...
let rec map_back_value
    ~(name : string)
    (sol : Solution.t)
    (map_back_transformer : map_back_transformer)
    (v : value)
    (orig_ty : ty)
  =
  ...
  | None ->
    ...
    | (VOption _ | VTuple _ | VRecord _ | VMap _), _ ->
      failwith
      @@ Printf.sprintf
           "%s: map_back_value: value %s does not match type %s"
           name
...
let rec map_back_mask
    ...
    (orig_ty : ty)
  =
  let map_back_mask = map_back_value ~name sol mask_transformer in
  ...
  | None ->
    (match v.v, orig_ty with
    | VBool _, (TUnit | TBool | TInt _ | TNode) | VOption None, TOption _ -> v
    ...
    | VRecord vmap, TRecord tmap ->
      vrecord @@ StringMap.mapi (fun l v -> map_back_mask v (StringMap.find l tmap)) vmap
    | VMap bdd, TMap (_, vty) ->
      let op_key = e_val v, BatSet.PSet.empty in
      vmap (BddMap.map op_key (fun v -> map_back_mask v vty) bdd)
    | (VUnit | VInt _ | VNode _ | VEdge _), _ ->
      failwith @@ name ^ ": Found illegal mask value"
    | VClosure _, _ -> failwith @@ name ^ ": Can't have closures in mask"
...
let map_back_sol
    ...
    (map_back_transformer : map_back_transformer)
    (mask_transformer : mask_transformer)
    (symb_tys : ty VarMap.t)
    (solve_tys : ty VarMap.t)
    (sol : Solution.t)
    : Solution.t
  =
  (* print_endline name; *)
  ...
  { symbolics =
      List.map
        (fun (x, v) ->
          try x, map_back_value v (VarMap.find x symb_tys) with
          | Not_found -> x, v)
        sol.symbolics
...
let transform_declarations
  ...
  , map_back_sol ~name map_back_transformer mask_transformer symb_tys solve_tys )
;;
