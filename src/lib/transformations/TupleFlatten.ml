...
let ty_transformer (recursors : Transformers.recursors) ty =
  ...
  | TArrow (t1, t2) ->
    ...
    begin
      match t1 with
      | TTuple tys ->
        Some (List.fold_right (fun t acc -> TArrow (t, acc)) tys t2)
      | _ -> Some (TArrow (t1, t2))
    end
  | _ -> None
;;
...
let exp_transformer (recursors : Transformers.recursors) e =
  ...
  | EVar x ->
    begin
      ...
      | TTuple ts ->
        let es =
          List.mapi
            (fun i ty -> aexp (evar (proj_var i x), Some ty, Span.default))
            ts
        in
        Some (etuple es)
      | _ -> None
      ...
      | _ ->
        begin
          ...
          | TTuple ts ->
            ...
            Some
              (ematch e1 (addBranch (PTuple ps) (flatten_exp e2) emptyBranch))
  ...
  | ETuple es ->
    ...
    let rec build_exp lst acc =
      ...
      | hd :: tl ->
        curry_elts hd (fun es -> build_exp tl (List.rev_append es acc))
    in
    Some (build_exp es [])
  | EOp (op, es) ->
...
let unproj_sym_solve (map : (Var.t * 'a) list) =
  let map =
    List.fold_left (fun acc (k, v) -> VarMap.add k v acc) VarMap.empty map
  in
  let unboxed_map =
    VarMap.fold
      (fun var v acc ->
    ...
    VarMap.fold
      (fun var lst acc ->
        ...
        | _ ->
          ...
          (var, { sol_val; mask; attr_ty }) :: acc)
      (*NOTE: type here might be wrong *)
