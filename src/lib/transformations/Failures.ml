...
let failuresConstraint_arith
  ...
  let bool2int_exp arg =
    aexp
      ( eif arg (e_val (vint one)) (e_val (vint zero))
      , Some Typing.node_ty
      , Span.default )
  in
  let failuresSum =
      ...
      (fun acc (uhat, vhat) ->
        aexp
          ( eop
              (UAdd 32)
              [ bool2int_exp
                  (evar (AdjGraph.EdgeMap.find (uhat, vhat) failuresMap))
              ; acc ]
          , Some Typing.node_ty
          , Span.default ))
      (e_val (vint zero))
  ...
  aexp
    ( eop
        (ULeq 32)
        [failuresSum; e_val (vint (Integer.create ~value:k ~size:32))]
    , Some TBool
    , Span.default )
;;

let failuresConstraint_pb (k : int) (failuresMap : Var.t AdjGraph.EdgeMap.t)
    : Syntax.exp
  =
  let arg1 =
    aexp
      ...
      , Some
          (TTuple
             (BatList.init (AdjGraph.EdgeMap.cardinal failuresMap) (fun _ ->
                  TBool)))
      , Span.default )
  in
  let arg2 =
      ...
      ( etuple
          (AdjGraph.EdgeMap.fold
             (fun _ _ acc ->
               exp_of_value
                 (avalue
                    (vint (Integer.of_int 1), Some (TInt 32), Span.default))
               :: acc)
             failuresMap
             [])
      , Some
  ...
  let arg3 =
    exp_of_value
      (avalue (vint (Integer.of_int k), Some (TInt 32), Span.default))
  in
  aexp
...
let buildSymbolicFailures (aedges : AdjGraph.Edge.t list) (k : int) =
  ...
  let failuresMap =
    BatList.fold_left
      (fun acc (u, v) ->
        let e =
          Printf.sprintf "%s-%s" (Vertex.to_string u) (Vertex.to_string v)
        in
