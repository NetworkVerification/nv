...
let failuresConstraint_arith
    (k : int)
    (aedges : AdjGraph.Edge.t list)
  ...
  let bool2int_exp arg =
    aexp
      (eif arg (e_val (vint one)) (e_val (vint zero)), Some Typing.node_ty, Span.default)
  in
  let failuresSum =
    BatList.fold_left
      (fun acc (uhat, vhat) ->
  ...
  in
  aexp
    ( eop (ULeq 32) [failuresSum; e_val (vint (Integer.create ~value:k ~size:32))]
    , Some TBool
    , Span.default )
;;
    ...
    aexp
      ( etuple
          (AdjGraph.EdgeMap.fold
             (fun _ fv acc -> aexp (evar fv, Some TBool, Span.default) :: acc)
             failuresMap
             [])
      , Some
          (TTuple (BatList.init (AdjGraph.EdgeMap.cardinal failuresMap) (fun _ -> TBool)))
      , Span.default )
  in
  let arg2 =
    aexp
      ( etuple
          (AdjGraph.EdgeMap.fold
             (fun _ _ acc ->
               exp_of_value
                 (avalue (vint (Integer.of_int 1), Some (TInt 32), Span.default))
               :: acc)
             failuresMap
             [])
      , Some
          (TTuple
             (BatList.init (AdjGraph.EdgeMap.cardinal failuresMap) (fun _ -> TInt 32)))
      , Span.default )
  in
  let arg3 =
    exp_of_value (avalue (vint (Integer.of_int k), Some (TInt 32), Span.default))
  in
  aexp
    ( eop (AtMost (AdjGraph.EdgeMap.cardinal failuresMap)) [arg1; arg2; arg3]
    , Some TBool
    , Span.default )
;;
...
let buildSymbolicFailures (aedges : AdjGraph.Edge.t list) (k : int) =
  ...
  ( failuresMap
  , EdgeMap.fold (fun _ fvar acc -> (fvar, Ty TBool) :: acc) failuresMap []
  , failures_leq_k )
;;
...
let buildFailTrans
  ...
  let failCheck fvar body =
    aexp
      ( eif
          (aexp (evar fvar, Some TBool, Span.default))
  ...
  let branches =
    Hashtbl.fold
      (fun (u, v) transuv acc ->
        ...
        addBranch
          p
          (addFailureCheck (AdjGraph.EdgeMap.find (u, v) failuresMap) transuv)
          acc)
      trans
      emptyBranch
  in
  ...
  let match_exp =
    aexp
      ( ematch
          (aexp (evar aedge_var, Some Typing.edge_ty, Span.default))
  ...
  let trans_hat_msg =
    efunc
      { arg = messageArg; argty = transf.argty; resty = transf.resty; body = match_exp }
  in
  (*return fun e_hat m -> trans_hat_body *)
  efunc
    { arg = aedge_var
    ; argty = Some Typing.edge_ty
    ; resty = transuv.ety
    ; body = trans_hat_msg
    }
;;
