...
let rec combine_depresults r1 r2 =
  ...
  | _ ->
    failwith
    @@ "Cannot combine different kinds of depresults: "
    ^ depresult_to_string r1
    ^ " and "
    ^ depresult_to_string r2
;;
...
let rec compute_dependencies_exp (acc : depmap) (e : exp) : depresult =
  match e.e with
  | EVar x ->
    (try VarMap.find x acc with
    | _ -> DBase (DepSet.singleton (VarDep x)))
  | EVal v ->
    begin
      match v.v with
  ...
  | ELet (x, e1, e2) ->
    ...
    let acc = VarMap.add x e1_deps acc in
    (* TODO: Unroll any VarDeps? I don't think so. *)
  ...
  | EIf (test, tru, fls) ->
    ...
    in
    combine_depresults tru_deps fls_deps |> add_to_depresult test_deps
  | EMatch (test, branches) ->
    ...
    List.fold_left
      ...
      (List.tl branch_deps)
  | ESome _ ->
    failwith "Options must be unboxed before doing dependency analysis"
...
and compute_dependencies_branch acc test_deps (pat, body) : depresult =
  ...
  let rec get_deps_from_pat (acc, global_deps) test_deps p =
    ...
    | PTuple ps ->
      ...
      List.fold_left
        (fun acc' (tdep, p) -> get_deps_from_pat acc' tdep p)
        (acc, global_deps)
        lst
    | POption _ | PRecord _ ->
      failwith "Found option or record pattern during dependency computation"
    | PEdge _ ->
      failwith "Edge patterns should have been removed during tuple flattening"
  in
  let acc, global_deps = get_deps_from_pat (acc, DepSet.empty) test_deps pat in
  compute_dependencies_exp acc body |> add_to_depresult global_deps
...
and compute_dependencies_op acc op es : depresult =
  ...
  | NLeq, [e1; e2] ->
    combine_depresults
      (compute_dependencies_exp acc e1)
      (compute_dependencies_exp acc e2)
  | Eq, [e1; e2] ->
  ...
  | TSet (_, lo, hi), [e1; e2] ->
    ...
    if lo = hi
    then DTuple (List.modify_at lo (fun _ -> new_deps) odeps)
    else (
