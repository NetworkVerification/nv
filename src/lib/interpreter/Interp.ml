...
let rec interp_exp env e =
  ...
  | EVar x ->
    ...
    | None ->
      failwith
        (Printf.sprintf
           "runtime exception - unbound variable: %s"
           (Var.to_string x))
    | Some v -> v)
  | EVal v -> v
...
and interp_op env ty op es =
  ...
  | ( MMerge
    , { v = VClosure (c_env, f) } :: { v = VMap m1 } :: { v = VMap m2 } :: rest
    ) ->
    let seen = BatSet.PSet.singleton ~cmp:Var.compare f.arg in
  ...
  | ( MMapIte
    ...
    let seen0 =
      Syntax.free (BatSet.PSet.singleton ~cmp:Var.compare p.arg) p.body
    in
    let usedValEnv =
      Env.filter c_env0.value (fun x _ -> BatSet.PSet.mem x seen0)
    in
  ...
  | _, _ ->
    failwith
      (Printf.sprintf "bad operator application: %s" (Printing.op_to_string op))
