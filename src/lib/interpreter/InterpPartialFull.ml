...
let rec interp_exp_partial (env : Syntax.exp Env.t) e =
  ...
  | EIf (e1, e2, e3) ->
    ...
    then (
      ...
      , aexp
          ( eif
              pe1
              (snd (interp_exp_partial env e2))
              (snd (interp_exp_partial env e3))
          , e.ety
          , e.espan ) )
  | ELet (x, e1, e2) ->
    let _, pe1 = interp_exp_partial env e1 in
  ...
  | ETuple es ->
    ( env
    , aexp
        ( etuple (BatList.map (fun e -> snd (interp_exp_partial env e)) es)
        , e.ety
        , e.espan ) )
  | ESome e' ->
    env, aexp (esome (snd (interp_exp_partial env e')), e.ety, e.espan)
  | EMatch (e1, branches) ->
    let _, pe1 = interp_exp_partial env e1 in
    ...
    | Delayed ->
      ...
      , aexp
          ( ematch
              pe1
              (mapBranches
                 (fun (p, e) -> p, snd (interp_exp_partial env e))
                 branches)
          , e.ety
          , e.espan ) ))
...
and interp_op_partial env ty op es =
  ...
  then simplify_exps op pes
    ...
    | ( MMapFilter
      ...
      let seen1 =
        Syntax.free (BatSet.PSet.singleton ~cmp:Var.compare f1.arg) f1.body
      in
      let usedValEnv =
        Env.filter c_env1.value (fun x _ -> BatSet.PSet.mem x seen1)
      in
      ...
      let mtbdd =
        ...
        | None ->
          ...
          let env =
            Env.update
              (Env.map usedValEnv (fun v -> BddFunc.eval_value v))
              f1.arg
              bddf
          in
