...
let isMapOp op =
  match op with
  | MCreate | MGet | MSet | MMap | MMerge | MFoldNode | MFoldEdge | MMapFilter
    -> true
  | _ -> false
;;
...
let rec interp_exp_partial_opt isapp env expEnv e =
  ...
  | EApp (e1, e2) ->
    ...
    | EFun f when is_value pe2 ->
      interp_exp_partial_opt
        false
        (update_value env f.arg (to_value pe2))
        expEnv
        f.body
  ...
  | ELet (x, e1, e2) ->
    ...
    then
      ...
      aexp
        (elet x pe1 (interp_exp_partial_opt false env expEnv e2), e.ety, e.espan)
  | ETuple es ->
    aexp
      ( etuple (BatList.map (interp_exp_partial_opt false env expEnv) es)
      , e.ety
      , e.espan )
  | ESome e' ->
    aexp (esome (interp_exp_partial_opt false env expEnv e'), e.ety, e.espan)
  | EMatch (e1, branches) ->
    ...
    then (
      ...
      | Some (env2, e) ->
        interp_exp_partial_opt false { env with value = env2 } expEnv e
      | None ->
        failwith
          ("value "
  ...
  | ERecord _ | EProject _ ->
    failwith "Record found during partial interpretation"
...
let rec interp_exp_partial isapp env e =
  ...
  | EFun f ->
    ...
    then e
      ...
      aexp
        ( efun { f with body = interp_exp_partial false env f.body }
        , e.ety
        , e.espan )
  | EApp (e1, e2) ->
    ...
    then (
      ...
      aexp
        ( ematch
            pe1
            (mapBranches
               (fun (p, e) -> p, interp_exp_partial false env e)
               branches)
        , e.ety
        , e.espan )
    (* |> simplify_match *)
