...
type 'a queue = Queue of ('a -> 'a -> int) * 'a list * 'a list * 'a PSet.t

let empty cmp = Queue (cmp, [], [], PSet.create cmp)

let add q elt =
  match q with
  | Queue (cmp, front, back, set) ->
    if PSet.mem elt set then q else Queue (cmp, elt :: front, back, PSet.add elt set)
;;
...
let pop q =
  match q with
  | Queue (_, [], [], _) -> None
  | Queue (cmp, front, b :: bs, set) -> Some (b, Queue (cmp, front, bs, PSet.remove b set))
  | Queue (cmp, front, [], set) ->
