...
module HashTblValue : Hashtbl.S with type key = Syntax.value =
  Hashtbl.Make (HashValue)
...
let map_when ~op_key (pred : bool Mtbdd.t) (f : value -> value) ((vdd, ty) : t)
    : t
  =
  let cfg = Cmdline.get_cfg () in
  let g b v =
...
let forall ~op_key (pred : bool Mtbdd.t) (f : value -> value) ((vdd, _) : t)
    : value
  =
  let cfg = Cmdline.get_cfg () in
  let g b v =
...
let merge ?opt ~op_key (f : value -> value -> value) ((x, tyx) : t) ((y, _) : t)
    : t
  =
  let cfg = Cmdline.get_cfg () in
...
let splitArray vars i j elt =
  Array.init (Array.length vars) (fun k ->
      if k >= i && k <= j then elt else vars.(k))
;;
  ...
  then acc
    ...
    | Man.Top ->
      ...
      bintRange
        ...
        (List.map
           (fun arr ->
             [splitArray arr i i Man.False; splitArray arr i i Man.True])
           acc
        |> List.concat))
;;
