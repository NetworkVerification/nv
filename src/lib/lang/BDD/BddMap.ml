...
let rec default_value ty =
  ...
  | TInt size -> avalue (vint (Integer.create ~value:0 ~size), Some ty, Span.default)
  | TRecord map ->
    avalue
      ( vtuple (BatList.map default_value @@ RecordUtils.get_record_entries map)
      , Some ty
      , Span.default )
  | TTuple ts -> avalue (vtuple (BatList.map default_value ts), Some ty, Span.default)
...
let value_to_bdd (v : value) : Bdd.vt =
  let rec aux v idx =
    ...
    | VInt i -> B.mk_int i idx, idx + Integer.size i
    | VTuple vs ->
    ...
    | VOption (Some dv) ->
      ...
      let value, idx = aux dv (idx + 1) in
      Bdd.dand tag value, idx
    | VMap _ | VClosure _ -> failwith "internal error (value_to_bdd)"
  in
...
let vars_to_value vars ty =
  ...
  let rec aux idx ty =
    let v, i =
      ...
      | TBool -> vbool (B.tbool_to_bool vars.(idx)), idx + 1
      | TInt size ->
      ...
      | TOption tyo ->
        ...
        v, i
      | TArrow _ | TMap _ | TVar _ | QVar _ -> failwith "internal error (bdd_to_value)"
    in
    let ty =
      match ty with
...
let bindings ((map, ty) : t) : (value * value) list * value =
  ...
  Mtbdd.iter_cube
    (fun vars v ->
      ...
      let expanded = if B.count_tops vars sz <= 2 then B.expand lst sz else [lst] in
      List.iter
        (fun vars ->
          if not (equal_values ~cmp_meta:false v dv)
          then (
...
module HashValue : Hashtbl.HashedType with type t = Syntax.value = struct
  type t = Syntax.value
...
let bindings_all ((map, ty) : t) : (value * value) list =
  ...
  Mtbdd.iter_cube
    (fun vars v ->
      if not (HashTblValue.mem seen v)
      then begin
...
let forall ~op_key (pred : bool Mtbdd.t) (f : value -> value) ((vdd, _) : t) : value =
  ...
  let op =
    ...
    | None ->
      ...
      let op =
        User.make_op2
          ~memo:Cudd.Memo.Global (* ~memo:(Memo.Cache (Cache.create2 ~maxsize:4096 ())) *)
          ~commutative:false
          ~idempotent:false
          ~special
          g
...
let splitArray vars i j elt =
  Array.init (Array.length vars) (fun k -> if k >= i && k <= j then elt else vars.(k))
;;
...
let rec bintRange i vars acc =
  ...
  then acc
    ...
    match vars.(i) with
    | Man.True | Man.False -> bintRange (i + 1) vars acc
    | Man.Top ->
      (* let j = ref (i-1) in
      ...
      bintRange
        ...
        vars
        (List.map
           (fun arr -> [splitArray arr i i Man.False; splitArray arr i i Man.True])
           acc
        |> List.concat))
;;
...
let createRanges i vars =
  let rec aux acc =
    ...
    | [] -> []
    | [x] ->
      let x = int_of_bint x in
      [x, x]
    | x :: y :: acc -> (int_of_bint x, int_of_bint y) :: aux acc
  in
...
let vars_to_multivalue vars ty =
  ...
  let rec aux idx ty =
    let v, i =
      match get_inner_type ty with
      | TUnit -> MUnit, idx + 1
      | TBool -> MBool (B.tbool_to_obool vars.(idx)), idx + 1
      | TInt size ->
...
let rec multiValue_to_string (mv : multiValue) =
  ...
  | MInt rs ->
    PrimitiveCollections.printList
      ...
      "}"
  | MBit bs ->
    String.init (Array.length bs) (fun i ->
        match bs.(i) with
        | None -> '*'
        | Some true -> '1'
        | Some false -> '0')
  ...
  | MTuple ts -> PrimitiveCollections.printList multiValue_to_string ts "[" "; " "]"
  | MNode rs -> Printf.sprintf "%sn" (multiValue_to_string (MBit rs))
  | MEdge _ -> failwith "Todo printing of edges"
;;
