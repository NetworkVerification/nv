...
let eqMap x y : Bdd.vt =
  ...
  | BMap m, Value v | Value v, BMap m ->
    Mapleaf.mapleaf1
      (fun vm -> equal_values ~cmp_meta:false (Mtbdd.get vm) v |> Mtbdd.unique B.tbl_bool)
      (fst m)
    |> bdd_of_mtbdd
...
let rec eq (x : t) (y : t) : t =
  ...
  then (*lift to mtbdd*)
    ...
    match x, y with
    | Value v1, Value v2 ->
      Value (if equal_values ~cmp_meta:false v1 v2 then vbool true else vbool false)
    ...
    | Tuple bs, Value v | Value v, Tuple bs ->
      (match v.v with
      | VTuple vs -> eq_list bs (List.map (fun v -> Value v) vs) (Value (vbool true))
      | VEdge (n1, n2) ->
  ...
  | Value vs, Value v ->
    ...
    | VTuple elts ->
      ...
      then Value (vtuple (BatList.modify_at lo (fun _ -> v) elts))
        ...
        match v.v with
        | VTuple velts -> Value (vtuple (OCamlUtils.replaceSlice lo hi elts velts))
        | _ -> failwith "Bad TSet")
    | _ -> failwith "Expected a tuple")
  | Value vs, x ->
    (match vs.v with
...
and eval_branch env (g : t) p =
  ...
  | PBool b, Value v ->
    ...
    | _ -> None)
  | PBool b, BBool bv -> if b then Some (env, Some bv) else Some (env, Some (Bdd.dnot bv))
  | PBool b, BMap m ->
    let cond =
      Mapleaf.mapleaf1
        (fun vm ->
...
and eval_ite_bdd_guard key_ty b v1 v2 =
  ...
  | Tuple bs1, Value v2 ->
    (match v2.v with
    | VTuple vs2 -> Tuple (eval_ite_list key_ty b bs1 (List.map (fun v -> Value v) vs2))
    | _ -> failwith "expected a tuple of values")
  | Value v1, Tuple bs2 ->
    (match v1.v with
    | VTuple vs1 -> Tuple (eval_ite_list key_ty b (List.map (fun v -> Value v) vs1) bs2)
    | _ -> failwith "expected a tuple of values")
