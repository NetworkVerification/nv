...
let get_start_position (span : Nv_datastructures.Span.t) info = get_position_opt span.fname span.start info

let get_end_position (span : Nv_datastructures.Span.t) info = get_position_opt span.fname span.finish info
...
let show_message_position info (span: Nv_datastructures.Span.t) msg color label =
  let border = "\n" in
