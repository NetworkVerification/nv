...
let rec default_value_exp ty =
  ...
  | TInt size ->
    exp_of_value (avalue (vint (Integer.create ~value:0 ~size), Some ty, Span.default))
  | TTuple ts -> aexp (etuple (BatList.map default_value_exp ts), Some ty, Span.default)
  | TRecord map ->
    aexp
      ( etuple (BatList.map default_value_exp @@ RecordUtils.get_record_entries map)
      , Some ty
      , Span.default )
  | TOption _ -> exp_of_value (avalue (voption None, Some ty, Span.default))
  | TMap (_, ty2) -> aexp (eop MCreate [default_value_exp ty2], Some ty, Span.default)
  | TVar { contents = Link t } -> default_value_exp t
...
let rec random_value ~hints ~max_map_size ty =
  ...
  | _ ->
    ...
    | TInt size ->
      let x = Integer.create_64 ~value:(Random.int64 Int64.max_int) ~size in
      vint x
    | TTuple ts -> vtuple (List.map (fun ty -> random_value hints max_map_size ty) ts)
    | TOption ty ->
      let b = Random.bool () in
      if b then voption None else voption (Some (random_value hints max_map_size ty))
    | TMap (ty1, ty2) ->
