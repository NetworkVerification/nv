...
let rec equal_values ~cmp_meta (v1 : value) (v2 : value) =
  ...
  let b =
    if cfg.hashcons then v1.vtag = v2.vtag else equal_vs ~cmp_meta v1.v v2.v
  in
  if cmp_meta
...
and equal_exps ~cmp_meta (e1 : exp) (e2 : exp) =
  let cfg = Cmdline.get_cfg () in
  let b =
    if cfg.hashcons then e1.etag = e2.etag else equal_es ~cmp_meta e1.e e2.e
  in
  if cmp_meta
...
and hash_e ~hash_meta e =
  ...
  | EFun f ->
    let i =
      if hash_meta
      then hash_opt hash_ty f.argty + hash_opt hash_ty f.resty
      else 0
    in
...
let rec exp_of_value v =
  match v.v with
  | VUnit | VBool _ | VInt _ | VMap _ | VClosure _
  | VOption None
  | VNode _ | VEdge _ ->
    let e = e_val v in
...
let get_graph ds =
  ...
  | Some n, Some es ->
    Some (List.fold_left AdjGraph.add_edge_e (AdjGraph.create n) es)
  | _ -> None
;;
