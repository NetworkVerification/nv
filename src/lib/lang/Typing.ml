    ...
    then true (* t1 and t2 are physically the same *)
      ...
      | TRecord map1, TRecord map2 ->
        ...
        then false
          ...
          BatList.fold_left
            (fun b l ->
              b && try_unify (StringMap.find l map1) (StringMap.find l map2))
            true
...
let op_typ op =
  ...
  | Eq ->
    failwith
      (Printf.sprintf "internal error (op_typ): %s" (Printing.op_to_string op))
;;
  ...
  let exp =
    ...
    | EOp (o, es) ->
      ...
      | TGet (size, lo, hi), [e1] ->
        ...
        let elt_tyvars =
          List.map (fun _ -> fresh_tyvar ()) (List.make size ())
        in
