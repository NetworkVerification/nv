...
let show_list s ls = "[" ^ List.fold_left (fun acc x -> acc ^ "," ^ s x) "" ls ^ "]"

let show_record f prefix map =
  let str =
    Nv_utils.PrimitiveCollections.StringMap.fold
...
let rec show_ty ?(show_links = false) ty =
  ...
  | TVar tyvar ->
    (match !tyvar with
    | Unbound (name, _) -> Printf.sprintf "TVar (Unbound %s)" (Var.to_string name)
    | Link t ->
      if show_links
      then Printf.sprintf "Link (%s)" (show_ty t)
      else Printf.sprintf "%s" (show_ty t))
  | QVar name -> Printf.sprintf "QVar (%s)" (Var.to_string name)
  | TBool -> "TBool"
  | TInt n -> Printf.sprintf "TInt %d" n
  | TArrow (ty1, ty2) -> Printf.sprintf "TArrow (%s,%s)" (show_ty ty1) (show_ty ty2)
  | TTuple ts ->
    let str = show_list show_ty ts in
    Printf.sprintf "TTuple %s" str
  | TOption t -> Printf.sprintf "TOption (%s)" (show_ty t)
  | TMap (ty1, ty2) -> Printf.sprintf "TMap (%s,%s)" (show_ty ty1) (show_ty ty2)
...
let rec show_exp ~show_meta e =
  ...
  then
    Printf.sprintf
      "{e=%s; ety=%s; espan=%s; etag=%d; ehkey=%d}"
      (show_e ~show_meta e.e)
      (show_opt show_ty e.ety)
      (Span.show_span e.espan)
      e.etag
      e.ehkey
  else Printf.sprintf "e=%s" (show_e ~show_meta e.e)
...
and show_e ~show_meta e =
  ...
  | EOp (op, es) ->
    Printf.sprintf "EOp (%s,%s)" (show_op op) (show_list (show_exp ~show_meta) es)
  ...
  | EIf (e1, e2, e3) ->
    Printf.sprintf
      "EIf (%s,%s,%s)"
      (show_exp ~show_meta e1)
      (show_exp ~show_meta e2)
...
and show_pattern p =
  ...
  | PTuple ps -> Printf.sprintf "PTuple %s" (show_list show_pattern ps)
  | POption None -> "POption None"
  | POption (Some p) -> Printf.sprintf "POption (Some %s)" (show_pattern p)
  | PRecord map -> show_record show_pattern "PRecord" map
  | PNode node -> Printf.sprintf "PNode %d" node
  | PEdge (p1, p2) -> Printf.sprintf "PEdge %s~%s" (show_pattern p1) (show_pattern p2)
...
and show_value ~show_meta v =
  ...
  then
    Printf.sprintf
      ...
      (Span.show_span v.vspan)
      v.vtag
      v.vhkey
  else Printf.sprintf "{v=%s;}" (show_v ~show_meta v.v)
...
and show_v ~show_meta v =
  ...
  | VOption vo -> Printf.sprintf "VOption (%s)" (show_opt (show_value ~show_meta) vo)
