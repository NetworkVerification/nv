...
module Edge =
  struct
    ...
    let compare (v1,w1) (v2,w2) =
      if UInt32.compare v1 v2 != 0 then
	UInt32.compare v1 v2
      else
	UInt32.compare w1 w2
  end
...
let vertex_map_to_string elem_to_string m =
  ...
  List.fold_left (fun s (k,v) -> UInt32.to_string k ^ ":" ^ elem_to_string v ^ s) "" kvs 

let print_vertex_map elem_to_string m =
  VertexMap.iter (fun k v -> print_endline (UInt32.to_string k ^ ":" ^ elem_to_string v)) m
...
let edges (m,i) =
  ...
  in
  List.rev (VertexMap.fold (fun v neighbors a -> my_edges v neighbors @ a) m [])
...
let neighbors (m,i) v =
  good_vertex (m,i) v;
  match find_opt v m with
    | None -> []
    | Some ns -> ns

let print g =
  Printf.printf "%d\n"  (UInt32.to_int (num_vertices g));
  List.iter (fun (v,w) -> Printf.printf "%d -> %d\n" (UInt32.to_int v) (UInt32.to_int w)) (edges g)
