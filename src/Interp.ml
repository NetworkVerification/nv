...
let empty_env =
  {ty=Env.empty; value=Env.empty;}
  
let update_value env x v =
  {env with value = Env.update env.value x v;}

let update_values env venv =
  {env with value = Env.updates env.value venv;}
    
let update_ty env x t =
  {env with ty = Env.update env.ty x t;}
...
let rec interp_exp env e =
  match e with
	...
	let v1 = interp_exp env e1 in
	match v1 with
	  | VTyClosure (c_env, (tvs, body)) -> interp_exp (update_tys c_env tvs tys) body
	  | _ -> error "bad functional application"
	    end
...
and interp_op env op es =
  ...
  | MMerge, [(VClosure (c_env, f)); (VMap (m1,t1)); (VMap (m2,t2))] ->
      ...
      let f_lifted v1opt v2opt =
        match apply c_env f (VTuple [VOption (v1opt,None); VOption (v2opt,None)]) with
        | VOption (vopt,_) -> vopt
