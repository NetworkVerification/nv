...
let rec equal_val v1 v2 =
  ...
  | VRecord map1, VRecord map2 -> StringMap.equal equal_val map1 map2
...
let rec matches p (v: Syntax.value) : Syntax.value Env.t option =
  ...
  | PRecord pmap, VRecord vmap ->
    if not (same_labels pmap vmap)
    then None
    else
      let pentries = get_record_entries pmap in
      let ventries = get_record_entries vmap in
      matches_list pentries ventries
  | POption None, VOption None -> Some Env.empty
  | POption (Some p), VOption (Some v) -> matches p v
  | (PBool _ | PInt _ | PTuple _ | POption _ | PRecord _), _ -> None
...
let rec interp_exp env e =
  ...
  | ERecord map -> vrecord (StringMap.map (interp_exp env) map)
  ...
  | EProject (e, l) ->
    ...
    begin
      ...
      | VRecord map ->
        begin
          try
            StringMap.find l map
          with
          | _ ->
            failwith @@ "Record value " ^ value_to_string v ^
                        " does not contain label " ^ l
        end
      | _ -> failwith "bad record projection"
    end
  | EMatch (e1, branches) ->
...
let rec interp_exp_partial isapp env e =
  ...
  | ETuple es ->
    aexp (etuple (List.map (interp_exp_partial false env) es),
          e.ety, e.espan)
  | ERecord map ->
    aexp (erecord (StringMap.map (interp_exp_partial false env) map),
          e.ety, e.espan)
  | EProject (e1, l) ->
    let pe1 = interp_exp_partial false env e1 in
    if is_value pe1 then
      ...
      | VRecord map ->
        aexp
          (e_val (try StringMap.find l map
                  with | _ -> failwith "Bad record projection")
          , e.ety, e.espan)

      | _ -> failwith "Bad record projection"
    else
      aexp (eproject pe1 l, e.ety, e.espan)
  ...
  let rec matches p (e: Syntax.exp) : Syntax.exp Env.t isMatch =
    ...
    | PRecord pmap ->
      if is_value e then
        (match e.e with
         | ERecord emap ->
           if not (same_labels pmap emap) then NoMatch
           else
             matches_list
               (get_record_entries pmap)
               (get_record_entries emap)
               Env.empty
         | _ -> Delayed)
      else Delayed
    | POption None ->
  ...
  let rec interp_exp_partial (env: Syntax.exp Env.t) e =
    ...
    | ETuple es ->
      aexp (etuple (List.map (interp_exp_partial env) es),
            e.ety, e.espan)
    | ERecord map ->
      aexp (erecord (StringMap.map (interp_exp_partial env) map),
            e.ety, e.espan)
    | EProject (e1, l) ->
      let pe1 = interp_exp_partial env e1 in
      if is_value pe1 then
        ...
        | VRecord map ->
          let v =
            try StringMap.find l map
            with | _ -> failwith "bad record projection"
          in
          aexp (e_val v, e.ety, e.espan)
        | _ -> failwith "bad record projection"
      else
        aexp (eproject pe1 l, e.ety, e.espan)
