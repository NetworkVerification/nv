...
let rec inline_app env e1 e2 : exp =
  ...
  | ELet (x, e3, e4) ->
      ELet (x, e3, inline_exp env (EApp (e4, e2) |> wrap e4)) |> wrap e1
...
and inline_exp (env: exp Env.t) (e: exp) : exp =
  ...
  | ELet (x, e1, e2) ->
      ...
      else ELet (x, e1', inline_exp (Env.remove env x) e2) |> wrap e
  ...
  | EMatch (e1, bs) ->
      (* TODO: remove match variables from needing inlining *)
