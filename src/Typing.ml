...
(* new type variable *)
let fresh_tyvar () =
  TVar (ref (Unbound (Var.fresh "a", level())))
    
(* equality of type variables *)
let equal_tyvars tv1 tv2 =
  tv1 == tv2  (* physical equality of refs *)

let reset_tyvars () =       (* name from OCaml's typing/typetext.ml *)
  (*  Var.reset ();  *)     (* DPW: don't need to do this *)
  level_reset ()
  ...
  let rec gen ty =
    match ty with
      | TVar {contents = Unbound (name,l)} 
          when l > !current_level -> (Env.bind name (), QVar name)
...
let rec infer_exp env e =
  match e with
    ...
    | EOp (o, es) ->
      begin
	match o with
	  | MCreate _ | MGet | MSet | MMap | MMerge -> failwith "unimplemented map ops"
	  | _ ->
