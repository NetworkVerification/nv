...
let filename () =
  match !filename_flag with
      None -> print_endline "use -f <filename> to add nv file"; exit 0
    | Some f -> f
...
let main =
  ...
  if simulate() then (
    ...
    (match q with
	[] -> ()
      | qs -> print_string "non-quiescent nodes:"; List.iter (fun q -> print_string (Unsigned.UInt32.to_string q ^ ";")) qs);
