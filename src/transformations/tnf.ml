  ...
  | EIf (e1, e2, e3) ->
     ...
     (match (oget e.ety) with
      | TTuple _ ->
         ...
         aexp(etuple (BatList.map2 (fun e2 e3 -> eif e1 e2 e3 |> wrap e2) es2 es3), e.ety, e.espan)
     ...
     | ELet (x, e1, e2) ->
        ...
        (match oget e2.ety with
         | TTuple _ ->
            aexp (etuple (BatList.map (fun e2 -> elet x e1 e2 |> wrap e2) (tupleToList e2)), e.ety, e.espan)
         | _ ->
            aexp (elet x e1 e2,e.ety, e.espan))
     | ETuple es ->
        etuple (BatList.map tnf_exp es) |> wrap e
     | ESome e1 ->
        aexp (esome (tnf_exp e1), e.ety, e.espan)
     | EMatch (e1, bs) ->
        let es1 = tnf_exp e1 in
        let bs = mapBranches (fun (p,b) -> (p, tnf_exp b)) bs in
        (match oget e.ety with
         |  TTuple ts ->
             let es = BatList.mapi (fun i ty ->
                          aexp(ematch es1 (mapBranches (fun (p,b) -> (p, BatList.nth (tupleToList b) i)) bs),
