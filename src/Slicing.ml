               ...
               begin
                 ...
                 | VTuple [v1;v2] ->
                    (match v1.v, v2.v with
                     | VInt ip, VInt p ->
                        Printf.printf "pre: %s,%d\n" (Integer.to_string ip) (Integer.to_int p);
...
let relevantPrefixes (assertionTable : (Integer.t, Syntax.exp) Hashtbl.t) =
  Hashtbl.fold (fun _ eu acc ->
      Printf.printf "assertions: %s\n" (Printing.exp_to_string eu);
...
let partialEvalAssert (n : network) =
  Printf.printf "assertion before eval: %s\n" (Printing.exp_to_string n.assertion);
  Printf.printf "assertion before eval: %s\n" (Syntax.show_exp ~show_meta:false n.assertion);
