...
class NvFile:
    ...
    def add_topology(self, nodes, edges, names=None):
        """
        Add the topology.
...
def RoEduNet():
    ...
    return len(nodes), edge_list, business_rel, None


def from_relns(relns_file):
...
def to_nv(nodes, edges, business_rel, names, symb=False):
    """
    ...
    if symb:
        f.add_symb("d", "int")
        # requirement that d is not a provider
        allowed_nodes = " || ".join([f"d = {v}" for v in stubs(nodes, business_rel)])
...
if __name__ == "__main__":
    nodes, edges, business_rel, names = from_relns(
        f"{os.path.dirname(sys.argv[0])}/RoEduNet_relns.txt"
    )
    if sys.argv[1] == "nv":
        symb = sys.argv[2] == "-s" or sys.argv[2] == "--symbolic"
        print(to_nv(nodes, edges, business_rel, names, symb))
    elif sys.argv[1] == "hgr":
        if names is not None:
